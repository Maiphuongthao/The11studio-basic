import {
  beautyServiceImg,
  hairstro,
  photoHeader,
  pmuServiceImg,
  tattooServiceImg,
  trico,
} from "../assets";
import { homepage2 } from "../assets";
import { homepage3 } from "../assets";
import { homepage4 } from "../assets";

export const navigation = [
  {
    id: "1",
    title: "Prestations",
    url: "#studio-services",
    image: "",
    icon: true,
  },
  {
    id: "2",
    title: "tarifs",
    url: "#prices",
  },
  {
    id: "3",
    title: "Studio",
    url: "#aboutus",
  },
  {
    id: "4",
    title: "réservation",
    url: "#booking",
  },
];

export const services = [
  {
    id: "0",
    title: "beauté du regard",
    url: "#beauty",
    image: beautyServiceImg,
  },
  {
    id: "1",
    title: "maquillage permanent",
    url: "#pmu",
    image: pmuServiceImg,
  },
  {
    id: "2",
    title: "tattoo & piercing",
    url: "#tatoo&piercing",
    image: tattooServiceImg,
  },

  {
    id: "3",
    title: "tricopigmentation",
    url: "#scalpsmp",
    image: trico,
  },
];

export const links = [
  {
    id: "0",
    title: "nous contacter",
    url: "#contact",
  },
  {
    id: "1",
    title: "Réserver mon rendez-vous",
    url: "#treatwell",
  },
];

export const medias = [
  {
    id: "0",
    title: "Instagram",
    url: "#",
  },
  {
    id: "1",
    title: "Facebook",
    url: "#",
  },
  {
    id: "2",
    title: "Tiktok",
    url: "#",
  },
  {
    id: "3",
    title: "Telegram",
    url: "#",
  },
];
export const infos = {
  email: "bonjour@the11studio.fr",
  address: "2 Av. des Bosquets, 78180 Montigny-le-Bretonneux",
  telephone: "+33 6 59 01 83 05",
};

export const discounts = [
  { id: "0", title: "-10% pour votre premier tatouage", category: "tatouage" },
];

export const photos = [
  {
    id: "0",
    src: photoHeader,
    category: "tatouage",
    alt: "Photo tatouage sur un bras",
  },
  {
    id: "1",
    src: homepage2,
    alt: "miroir et chaises",
    category: "studio",
  },
  {
    id: "2",
    src: homepage3,
    alt: "chambre alumineuse avec tables de tatouages",
    category: "studio",
  },
  {
    id: "3",
    src: homepage4,
    alt: "coin reception client avec chaise et plant",
    category: "studio",
  },
];
export const actualities = [
  {
    id: "0",
    title: "MagicEffect brows",
    photo: hairstro,
    content:
      "Est laoreet quis aliquam ultrices quam integer. Aliquet cursus nisi egestas eu pellentesque amet suspendisse. Tincidunt neque amet dignissim euismod pulvinar risus enim fermentum. Blandit mauris aliquet senectus tincidunt. Donec nisl condimentum praesent id.",
    createdDate: "10/06/2024",
    url: "#",
  },
  {
    id: "1",
    title: "MagicEffect brows",
    photo: hairstro,
    content:
      "Est laoreet quis aliquam ultrices quam integer. Aliquet cursus nisi egestas eu pellentesque amet suspendisse. Tincidunt neque amet dignissim euismod pulvinar risus enim fermentum. Blandit mauris aliquet senectus tincidunt. Donec nisl condimentum praesent id.",
    createdDate: "10/06/2024",
    url: "#",
  },
  {
    id: "2",
    title: "MagicEffect brows",
    photo: hairstro,
    content:
      "Est laoreet quis aliquam ultrices quam integer. Aliquet cursus nisi egestas eu pellentesque amet suspendisse. Tincidunt neque amet dignissim euismod pulvinar risus enim fermentum. Blandit mauris aliquet senectus tincidunt. Donec nisl condimentum praesent id.",
    createdDate: "10/06/2024",
    url: "#",
  },
  {
    id: "3",
    title: "MagicEffect brows",
    photo: hairstro,
    content:
      "Est laoreet quis aliquam ultrices quam integer. Aliquet cursus nisi egestas eu pellentesque amet suspendisse. Tincidunt neque amet dignissim euismod pulvinar risus enim fermentum. Blandit mauris aliquet senectus tincidunt. Donec nisl condimentum praesent id.",
    createdDate: "10/06/2024",
    url: "#",
  },
  {
    id: "4",
    title: "MagicEffect brows",
    photo: hairstro,
    content:
      "Est laoreet quis aliquam ultrices quam integer. Aliquet cursus nisi egestas eu pellentesque amet suspendisse. Tincidunt neque amet dignissim euismod pulvinar risus enim fermentum. Blandit mauris aliquet senectus tincidunt. Donec nisl condimentum praesent id.",
    createdDate: "10/06/2024",
    url: "#",
  },
  {
    id: "5",
    title: "MagicEffect brows",
    photo: hairstro,
    content:
      "Est laoreet quis aliquam ultrices quam integer. Aliquet cursus nisi egestas eu pellentesque amet suspendisse. Tincidunt neque amet dignissim euismod pulvinar risus enim fermentum. Blandit mauris aliquet senectus tincidunt. Donec nisl condimentum praesent id.",
    createdDate: "10/06/2024",
    url: "#",
  },
];
